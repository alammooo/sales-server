import {
  ...
  getByIdService,
  updateService,
} from '@/services/master/district.services'
...
import {
  ...
  findValidator,
  updateValidator,
} from '@/validators/master/district.validators'

export const getAllController: ExpressFc = async (req, res, next) => {
  try {
    const { params } = await findValidator.validate(req)
    const result = await getAllService({ cityId: params.cityId })
    res.status(200).json({ message: 'success get all sub district', data: result })
  } catch (error) {
    next(error)
  }
}
